...
class TestServeLogs:
    ...
    def test_should_serve_file(self, client: TestClient, jwt_generator, monkeypatch):
        # Mock the JWT validation just for this test
        def mock_validated_claims(self, token, required_claims=None):
            return {"filename": "sample.log"}

        monkeypatch.setattr(
            "airflow.api_fastapi.auth.tokens.JWTValidator.validated_claims", mock_validated_claims
        )

        response = client.get(
            "/log/sample.log",
            headers={
                "Authorization": "mock-token",  # Token doesn't matter with mocked validation
            },
        )
        assert response.content.decode() == LOG_DATA
    ...
    def test_ok_with_short_future_skew(self, client: TestClient, jwt_generator, monkeypatch):
        # Mock the JWT validation just for this test
        def mock_validated_claims(self, token, required_claims=None):
            return {"filename": "sample.log"}
    ...
    def test_ok_with_short_past_skew(self, client: TestClient, jwt_generator, monkeypatch):
        # Mock the JWT validation just for this test
        def mock_validated_claims(self, token, required_claims=None):
            return {"filename": "sample.log"}
